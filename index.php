	...
	<?php if (have_posts()) : ?>
		<?php while (have_posts()) : the_post(); ?>
			<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
				...
				<ul class="list-inline post-meta">
					<?php if ( is_sticky() ) : ?>
							<li><span class="glyphicon glyphicon-bookmark"></span> <?php _e( 'Featured','lmsim' ); ?></li>
					<?php endif; ?>
		...
		<nav class="text-center">
			<?php if(!wp_is_mobile() && (get_previous_posts_link() || get_next_posts_link())){ ?>
				...
				<ul class="pagination"><?php par_pagenavi(2); ?></ul>
				
			<?php }else{ ?>
				<ul class="pager">
					<li class="previous"><?php next_posts_link( '<span class="meta-nav">&larr;</span> Older posts' ); ?></li>
					<li class="next"><?php previous_posts_link( 'Newer posts <span class="meta-nav">&rarr;</span>' ); ?></li>
				</ul>
			<?php } ?>
		</nav>
	<?php else : ?>
